...
export async function generateMoveCode(id: string, composition: IComposition) {
  try {
    ...
    const codeResult = await generateCode(adaptedComposition, primitives);
    const generatedCode = codeResult.fullSourceCode;
    
    console.log(`Successfully generated ${generatedCode.length} bytes of Move code`);
    
    // Add comment to indicate AI refinement
    const codeWithComment = process.env.GEMINI_API_KEY 
      ? `// This code was generated by Move Matrix and refined using AI for better quality, security, and efficiency\n${generatedCode}`
      : `// This code was generated by Move Matrix\n${generatedCode}`;
    ...
    revalidatePath(`/compositions/${id}`);
    return { success: true, code: codeWithComment };
  } catch (error: any) {
